...
var handleErrorLock = function(response) {
	let res = response.responseJSON
	let code = response.status
	...
	if(code == 422) {
        ...
        console.log(res)
        const marks=res.data.realizations
        for(m in marks){
            if(marks[m].evidence==null){
                $('.tr-'+marks[m].target_id).parents('tr').css("background-color","#D6EBFF")
                console.log($('.tr-'+marks[m].target_id).parents('tr'))
            }
        }
	} else{
        handleErrorDetail(response)
    }
}
